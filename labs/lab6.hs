...
nrFeliiSicilia list = sum [x | (Portocala soi x) <- list, ePortocalaDeSicilia (Portocala soi x)]
...
rasa (Caine _ rasaCatel) = Just rasaCatel
...
verifica (M lines) n = foldr (&&) True $ (map (\(L line) -> sum line == n) lines)
...
-- doarPozN :: Matrice -> Int -> [Linie]
doarPozN :: Matrice -> Int -> Bool
doarPozN (M lines) n = foldr (&&) True $ map (\(L line) -> foldr (&&) True $ map (> 0) line) $ filter (\(L line) -> length line == n) lines
